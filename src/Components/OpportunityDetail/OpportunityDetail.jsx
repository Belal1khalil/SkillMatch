import { faBookmark, faBuilding, faCheckCircle, faClock } from '@fortawesome/free-regular-svg-icons'
import { faArrowLeft, faBolt, faMapMarkerAlt, faMoneyBillWave, faShareAlt } from '@fortawesome/free-solid-svg-icons'
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
              <div className="flex items-start md:items-center gap-6">
                <div className="w-24 h-24 bg-white/10 backdrop-blur-md rounded-[2rem] border border-white/20 flex items-center justify-center text-white text-4xl shadow-2xl shrink-0">
                   {opportunity.logo ? (
                     <img src={opportunity.logo} alt={opportunity.company} className="w-16 h-16 object-contain" />
                   ) : (
        ...
        <div className="lg:col-span-2 space-y-8 animate-slide-up">
          <div className="bg-white rounded-[3rem] p-10 shadow-xl shadow-gray-200/50 border border-gray-100">
            ...
            <div className="prose prose-blue max-w-none text-gray-600 leading-relaxed font-medium">
              <p className="whitespace-pre-line">
                {opportunity.description || "We are looking for a talented professional to join our team. You will be responsible for high-impact work and collaboration with cross-functional teams."}
              </p>
            </div>
            ...
            {opportunity.requirements && (
              <div className="mt-12">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(Array.isArray(opportunity.requirements) ? opportunity.requirements : [opportunity.requirements]).map((req, i) => (
                    <li key={i} className="flex items-start gap-3 p-4 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-primary-200 transition-colors">
                      <FontAwesomeIcon icon={faCheckCircle} className="text-primary-500 mt-1 shrink-0" />
                      <span className="text-gray-600 font-bold text-sm leading-snug">{req}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="bg-white rounded-[3rem] p-10 shadow-xl shadow-gray-200/50 border border-gray-100">
            ...
            <div className="flex flex-wrap gap-4">
               {(opportunity.skills || ["Communication", "Teamwork", "Problem Solving"]).map((skill, i) => (
                 <div key={i} className="px-6 py-4 bg-gray-50 rounded-2xl border border-gray-100 text-gray-700 font-black text-xs uppercase tracking-widest hover:bg-primary-50 hover:border-primary-200 transition-all cursor-default">
                    {skill}
                 </div>
               ))}
            </div>
          ...
          <div className="bg-white rounded-[3rem] p-8 shadow-xl shadow-gray-200/50 border border-gray-100 sticky top-24">
            ...
            <div className="space-y-6 mb-10">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Offered Salary</p>
                  <p className="text-gray-900 font-black">{opportunity.salary || "Competitive"}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-gray-900 font-black">{opportunity.type || "Full-time"}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-gray-900 font-black">{opportunity.location || "Remote"}</p>
                </div>
              </div>
            </div>
            ...
            <button className="w-full py-6 bg-gray-900 text-white rounded-[2rem] font-black text-sm uppercase tracking-[0.2em] shadow-xl hover:bg-primary-600 hover:shadow-primary-600/40 transition-all active:scale-95 mb-6 group">
              Apply for Job
              <FontAwesomeIcon icon={faBolt} className="ml-3 text-amber-400 group-hover:scale-125 transition-transform" />
            </button>
            <p className="text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest">
              Posted on {new Date(opportunity.createdAt || Date.now()).toLocaleDateString()}
            </p>
          </div>
